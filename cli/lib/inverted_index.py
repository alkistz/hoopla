...
from .search_utils import BM25_B, BM25_K1, CACHE_DIR, load_movies, load_stop_wrods
...
class InvertedIndex:
    ...
    def save(self):
        ...
        self.__write_to_file(self.doc_lengths_path, self.doc_lengths)
    ...
    def load(self):
        ...
        if not os.path.exists(self.term_frequencies_path):
            raise FileExistsError("Term frequencies pickle does not exist")

        if not os.path.exists(self.doc_lengths_path):
            raise FileExistsError("Doc lengths pickle does not exist")
